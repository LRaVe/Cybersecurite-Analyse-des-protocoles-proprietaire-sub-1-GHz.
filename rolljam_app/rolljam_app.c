...
static void save_captured_signal(RollJamApp* app, uint8_t index) {
    ...
    furi_string_printf(filename, "%s/code_%lu_%d.sub", CAPTURES_DIR, 
                       (unsigned long)sig->timestamp, index);
                        ...
                        if(app->rx_write_index > MIN_SIGNAL_SAMPLES) {
                            ...
                            if(idx < MAX_CAPTURED_CODES) {
                                ...
                                if(app->captured[idx].samples) {
                                    memcpy(app->captured[idx].samples, app->rx_buffer, 
                                    ...
                                    FURI_LOG_I(TAG, "Code 1 captured: %u samples", 
...
static int32_t capture_worker_thread(void* context) {
    ...
    while(app->worker_running) {
        ...
        if(rssi > RSSI_THRESHOLD) {
            ...
            silence_count++;
            if(silence_count > 50) {
                if(app->rx_write_index > MIN_SIGNAL_SAMPLES && app->capture_count < MAX_CAPTURED_CODES) {
                    uint8_t idx = app->capture_count;
                    app->captured[idx].samples = malloc(app->rx_write_index * sizeof(int32_t));
                    if(app->captured[idx].samples) {
                        memcpy(app->captured[idx].samples, app->rx_buffer, 
                        ...
                        save_captured_signal(app, idx);
                        snprintf(app->status_msg, sizeof(app->status_msg), 
                                "Captured code %d (%u samples)", idx + 1, 
                                (unsigned int)app->rx_write_index);
...
static const char* rolljam_state_str(RollJamState state) {
    switch(state) {
        case RollJamIdle: return "IDLE";
        case RollJamArmed: return "ARMED";
        case RollJamJamming1: return "JAM+CAP 1";
        case RollJamWaiting2: return "WAIT PRESS 2";
        case RollJamJamming2: return "JAM+CAP 2";
        case RollJamReady: return "CODE READY!";
        case RollJamComplete: return "COMPLETE";
        default: return "???";
    }
}
...
static void render_callback(Canvas* canvas, void* ctx) {
    ...
    switch(app->mode) {
        ...
        case ModeRollJam: {
            ...
            furi_string_printf(line, "Codes: %lu | Jam: %s", 
                              (unsigned long)app->codes_captured, app->jam_active ? "ON" : "OFF");
        ...
        case ModeCapture: {
            ...
            furi_string_printf(line, "Signals: %lu | Captured: %lu", 
                              (unsigned long)app->signals_detected, (unsigned long)app->codes_captured);
            canvas_draw_str(canvas, 2, 32, furi_string_get_cstr(line));
