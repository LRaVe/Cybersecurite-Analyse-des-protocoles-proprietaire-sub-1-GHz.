...
// ========= DEBUT CODE FONCTIONNE : Envoi réception dans les deux cordes
...
static void PingPong_Process(void)
{
    ...
    switch (State)
    {
        case RX:
            ...
            if (RxBufferSize > 0)
            {
                /* Nouveau : afficher pour tout signal reçu */
                APP_LOG(TS_ON, VLEVEL_L, "Le signal recopié à été reçu !\n\r");
                APP_LOG(TS_ON, VLEVEL_L, "RX OK size=%d first=%02X %02X %02X %02X\n\r", RxBufferSize, BufferRx[0], BufferRx[1], BufferRx[2], BufferRx[3]);
...
void ReenvoyerDernierSignal(void)
{
    ...
    {
        /* Copier le signal dans BufferTx pour l'envoyer */
        memcpy(BufferTx, LastRxBuffer, LastRxSize);

        /* Facultatif : délai RX → TX si nécessaire */
        HAL_Delay(Radio.GetWakeupTime() + RX_TIME_MARGIN);

        APP_LOG(TS_ON, VLEVEL_L, "Je renvoie le signal :  Taille=%d\n\r", LastRxSize);

        /* Envoyer via le mécanisme standard */
        RadioSend();

        SignalDisponible = false; // Signal envoyé, plus disponible
