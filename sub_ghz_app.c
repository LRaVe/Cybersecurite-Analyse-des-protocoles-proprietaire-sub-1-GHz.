...
// Plus le nombre est bas, plus la sensibilité est élevée mais plus il y a de bruit
// Valeurs recommandées: -65 (faible bruit), -75 (moyen), -85 (sensible)
...
static int32_t subghz_worker_thread(void* context) {
    ...
    while(app->is_capturing) {
        ...
        // Si RSSI détecte un signal (> seuil de bruit configuré)
        if(rssi > RSSI_THRESHOLD) {
            if(!signal_active) {
                // Début d'un nouveau signal
                signal_active = true;
                signal_start_time = furi_get_tick();
                max_rssi = rssi;
                FURI_LOG_I(TAG, "Signal debut: RSSI=%.1f dBm", (double)rssi);
            } else {
                // Signal en cours - tracker le RSSI max
                if(rssi > max_rssi) {
                    max_rssi = rssi;
                }
            }
            ...
            // Signal terminé - enregistrer si on était en capture active
            if(signal_active) {
                signal_duration = furi_get_tick() - signal_start_time;
                app->signal_count++;

                FURI_LOG_I(
                    TAG,
                    "Signal fin: duree=%lu ms, RSSI max=%.1f dBm, count=%lu",
                    signal_duration,
                    (double)max_rssi,
                    app->signal_count);
                ...
                if(file_stream_open(
                    ...
                    if(packet_size > 0) {
                        ...
                        furi_string_cat_printf(line, " Duration=%lu\n", signal_duration);
                    } else {
                        // Enregistrer la détection avec durée
                        furi_string_printf(
                            line,
                            "RAW_Signal: RSSI_max=%.1f dBm, Duration=%lu ms\n",
                            (double)max_rssi,
                            signal_duration);
                    }
        ...
        furi_delay_ms(1); // Vérifier chaque 1ms pour capturer précisément la durée
    }
...
static void start_capture(SubGhzApp* app) {
    ...
    // Configurer la fréquence et le chemin RF
    furi_hal_subghz_set_frequency_and_path(MY_FREQUENCY);

    // Configurer le GPIO GDO0 du CC1101 en mode input pour capturer les données asynchrones
    furi_hal_gpio_init(&gpio_cc1101_g0, GpioModeInput, GpioPullNo, GpioSpeedVeryHigh);

    // Vider le buffer RX avant de commencer
    furi_hal_subghz_flush_rx();

    // Passer en mode réception
    furi_hal_subghz_rx();

    // Délai pour stabiliser la réception et laisser le RSSI se calibrer
    furi_delay_ms(50);

    FURI_LOG_I(
        TAG,
        "Radio en mode reception, RSSI initial: %.1f dBm",
...
static int32_t subghz_replay_thread(void* context) {
    ...
    if(file_stream_open(stream, SUBGHZ_RAW_FILE_PATH, FSAM_READ, FSOM_OPEN_EXISTING)) {
        ...
        FURI_LOG_I(TAG, "Fichier ouvert, lecture des donnees...");

        // Lire toutes les lignes du fichier
        while(stream_read_line(stream, line) && app->is_replaying) {
            const char* line_str = furi_string_get_cstr(line);

            // Sauter les lignes d'en-tête (Filetype, Version, Frequency, etc.)
            if(!header_done) {
                if(strstr(line_str, "Protocol:") != NULL) {
                    header_done = true;
                    FURI_LOG_I(TAG, "En-tetes passes, debut du traitement des signaux");
                }
                continue;
            }

            // Traiter les lignes RAW_Data (format hex: "RAW_Data: XX XX XX")
            if(strstr(line_str, "RAW_Data:") != NULL) {
                ...
                furi_hal_subghz_idle();
                furi_hal_subghz_tx();
                furi_delay_ms(50); // Durée de transmission
                furi_hal_subghz_idle();

                FURI_LOG_D(TAG, "Signal RAW_Data transmis #%lu", replay_count);
                furi_delay_ms(10); // Pause entre signaux
            }
            // Traiter les lignes RAW_Signal (format: "RAW_Signal: RSSI=...")
            else if(strstr(line_str, "RAW_Signal:") != NULL) {
                replay_count++;
                app->replay_count = replay_count;

                // Transmettre un pulse pour chaque signal détecté
                furi_hal_subghz_idle();
                furi_hal_subghz_tx();
                furi_delay_ms(100); // Pulse de 100ms
                furi_hal_subghz_idle();

                FURI_LOG_D(TAG, "Signal RSSI transmis #%lu", replay_count);
                furi_delay_ms(20); // Pause entre signaux
            }
        }
    ...
    FURI_LOG_I(TAG, "Replay termine avec succes");
    return 0;
}
...
int32_t sub_ghz_app(void* p) {
    ...
    while(running) {
        ...
        // Mettre à jour l'affichage si en cours de capture ou replay
        if(app->is_capturing || app->is_replaying) {
            view_port_update(app->view_port);
        }

        // Vérifier si le thread de replay est terminé
